...
class Menu
{
     ...
     * <code>
     *    $bh->addMenuItem($menuItemID, $menuItemName, $positionInMenu, $linkAttributes, $pkgHandle = false, $iconMenu='fa-certificate');
     * </code>
     * @$iconMenu: default 'fa-certificate'
     */
    public function addPageHeaderMenuItem(
        ...
        $linkAttributes,
        $pkgHandle = false,
        $iconMenu = 'fa-certificate'
    ) {
        $obj = new MenuItem($menuItemID, $menuItemName, $iconMenu, $positionInMenu, $linkAttributes, $pkgHandle);
...
class MenuItem
{
    ...
    public function __construct($handle, $name, $icon, $position, $linkAttributes, $pkgHandle = false)
    {
        $this->handle = $handle;
        $this->name = $name;
        $this->icon = $icon;
    ...
    protected function mapMenuItemFilePath($_file)
    {
        ...
        if ($this->pkgHandle) {
            if (!isset($file)) {
                $pkgHandle = $this->pkgHandle;
                $dirp = is_dir(
                    DIR_PACKAGES . '/' . $pkgHandle
                ) ? DIR_PACKAGES . '/' . $pkgHandle : DIR_PACKAGES_CORE . '/' . $pkgHandle;
                if (file_exists(
                    $dirp . '/' . DIRNAME_ELEMENTS . '/' . DIRNAME_ELEMENTS_HEADER_MENU . '/' . $handle . '/' . $_file
                )
                ) {
        ...
        if (!isset($file)) {
            if (file_exists(
                DIR_FILES_ELEMENTS_CORE . '/' . DIRNAME_ELEMENTS_HEADER_MENU . '/' . $handle . '/' . $_file
            )
            ) {
...
<li class="pull-right hidden-xs">
    <a ' . $attribs . ' id="ccm-page-edit-nav-' . $this->menuItem->getHandle() . '"><span class="spinner"><div class="double-bounce1"></div><div class="double-bounce2"></div></span>
        <i class="fa ' . $this->menuItem->getIcon() . '"></i>
        <span class="ccm-toolbar-accessibility-title ccm-toolbar-accessibility-title-' . $this->menuItem->getHandle() . '">
